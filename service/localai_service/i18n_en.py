...
EN: dict[str, str] = {
    ...
    "可以把清晰度调到 480p 或缩短时长后再试，": "Try 480p or a shorter clip, ",
    "可以换成标准清晰度或更小的模型后再试，": "Try the standard resolution or a smaller model, ",
    "可以缩短歌曲时长后再试，": "Try a shorter song, ",
    "显卡显存不够用了：这次生成需要的显存超过了显卡当前能提供的。{advice}也可以关掉游戏、视频等占用显卡的程序。": "The graphics card ran out of memory: this needs more video memory than the card has free right now. {advice}or close games and video apps that use the graphics card.",
    "电脑内存不够用了。{advice}也可以关掉一些正在运行的程序后再试。": "The computer ran out of memory. {advice}or close some running apps and try again.",
    "磁盘空间不足，结果没法保存。请清理一些空间后再试。": "The disk is full, so the result could not be saved. Free up some space and try again.",
    "显卡驱动中途停止了响应。请重试一次；如果经常出现，建议更新显卡驱动或重启电脑。": "The graphics driver stopped responding. Try again; if it keeps happening, update the graphics driver or restart the computer.",
    "没有找到可用的显卡加速。请安装显卡厂商的最新驱动（NVIDIA、AMD 或 Intel）后再试。": "No graphics acceleration was found. Install the latest driver from NVIDIA, AMD or Intel and try again.",
