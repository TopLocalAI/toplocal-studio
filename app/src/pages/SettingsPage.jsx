...
function moduleStatus(moduleId, features, models) {
  ...
  if (!supported.length) {
    ...
    return { tone: "muted", text: t("需要 {memory} 以上内存", { memory: TIER_NEED[need] }) };
  }
  ...
  if (missing.length) {
    return { tone: "warn", text: t("需下载模型 · {size}", { size: formatBytes(missing.reduce((s, m) => s + m.sizeBytes, 0)) }) };
  }
  return { tone: "ok", text: limited.length ? t("可以使用（{features}需更大内存）", { features: limited.join(t("、")) }) : t("可以使用") };
}
    ...
    <div className="device-card">
      <div className="device-head">
        ...
        <div className="device-title">
          ...
          <span>
            {system.os} {system.osVersion} · {t("{memory} GB 内存", { memory: system.memoryGb })} ·{" "}
            {system.backend === "metal" ? t("Apple Metal 加速") : t("{backend} 加速", { backend: system.backend.toUpperCase() })}
          </span>
        </div>
      </div>
      ...
      <div className="device-meters">
        <div className="meter">
          ...
          <small>{t("16 GB 可用音乐、语音、图片；24 GB 起解锁视频和精细修图")}</small>
        </div>
        <div className="meter">
          <div className="meter-label">
            <span>{t("磁盘")}</span>
            <strong>
              {t("剩余 {size} GB", { size: Math.round(system.diskFreeGb) })}
              {system.diskTotalGb ? <em> / {t("共 {size} GB", { size: Math.round(system.diskTotalGb) })}</em> : null}
            </strong>
          </div>
          <div className={lowDisk ? "meter-bar is-low" : "meter-bar"} aria-hidden="true">
            <i style={{ width: `${Math.round(used * 100)}%` }} />
          </div>
          <small>{lowDisk ? t("空间偏少：视频模型约需 31 GB") : t("模型按需下载，全部模型约 65 GB")}</small>
        </div>
      </div>
...
export function SettingsPage({ system, theme, onThemeChange, language, serviceLanguage, onLanguageChange, onRefresh, focus }) {
  ...
  useEffect(() => {
    if (serviceLanguage) refresh(); // model names and licenses come translated from the service
  }, [serviceLanguage, refresh]);
  useEffect(() => {
    ...
    <section className="settings-page">
      <header className="page-header">
        ...
        <button type="button" className="button button-secondary" onClick={onRefresh}>
          <ArrowClockwise size={17} /> {t("重新检测")}
        </button>
      </header>

      <div className="settings-section settings-general">
        <div className="general-item">
          <h2>{t("语言")}</h2>
          <div className="segmented segmented-inline" role="radiogroup" aria-label={t("语言")}>
            {LANGUAGES.map((l) => (
              <button key={l.id} type="button" role="radio" aria-checked={language === l.id} className={language === l.id ? "is-active" : ""} onClick={() => onLanguageChange(l.id)}>
                {l.label}
              </button>
            ))}
          </div>
        ...
        <div className="general-item">
          <h2>{t("外观")}</h2>
          <div className="segmented segmented-inline" role="radiogroup" aria-label={t("外观")}>
      ...
      <div className="settings-section">
        ...
        {system ? (
          ...
          <p className="empty-note">{t("正在连接本地引擎…")}</p>
        )}
      </div>
      ...
      <div className="settings-section">
        ...
        {prefs ? (
          <div className="download-prefs">
            ...
            <label ref={tokenRef}>
              <span>
                {t("Hugging Face 访问令牌")}
                {prefs.hasHfToken ? <em className="token-saved"> · {t("已保存")}</em> : null}
              </span>
              <div className="token-row">
                <input
                  type="password"
                  value={token}
                  placeholder={prefs.hasHfToken ? t("已保存，输入新令牌可替换") : "hf_…"}
              ...
              <small className="token-help">
                {t("只有下表中标着")} <span className="token-tag"><Key size={11} /> {t("需令牌")}</span>{" "}
                {t("的模型需要（{names}），其他模型直接下载。", {
                  names: models.filter((m) => m.needsToken).map((m) => m.name).join(t("、")) || t("当前没有"),
                })}{" "}
                {t("获取方法：登录 Hugging Face，打开模型页面同意许可证，再到")}{" "}
                <button type="button" className="link-button" onClick={() => openExternal(TOKEN_URL)}>Access Tokens</button>{" "}
                {t("创建一个 Read 类型的令牌，粘贴到这里。")}
              </small>
            </label>
          </div>
        ) : null}
                  ...
                  <td>
                    ...
                    {m.needsToken ? (
                      <button
                        type="button"
                        className="token-tag"
                        title={t("需要 Hugging Face 令牌：点击打开模型页面同意许可证")}
                    ...
                    {m.installed ? (
                      <>
                        <span className="status-ok">
                          <CheckCircle size={16} weight="fill" /> {t("已安装")}
                        </span>
                        {confirming === m.id ? (
                          <>
                      ...
                      <>
                        ...
                        <small>{d.state === "verifying" ? t("校验中") : `${formatBytes(d.doneBytes)} / ${formatBytes(d.totalBytes)}`}</small>
                      ...
                      <>
                        {d?.state === "error" ? <small className="status-error" title={d.error}>{t("下载失败")}</small> : null}
        ...
        <p className="settings-note">
          {t("已安装模型共 {size}。所有生成都在本机完成，不上传任何内容。", { size: formatBytes(installedBytes) })}
        </p>
      </div>
      ...
      <div className="settings-section" ref={aboutRef}>
        ...
        <div className="about-card">
          ...
          <div>
            <strong>{t("TopLocal Studio · 本地创作台")}</strong>
